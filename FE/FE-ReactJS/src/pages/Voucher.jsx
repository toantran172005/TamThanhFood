...
function Voucher() {
  ...
  const formatVoucherData = (item) => {
    ...
    if (item.expiredAt) {
      const [day, month, year] = item.expiredAt.split("-");
      
      const expirationDate = new Date(`${year}-${month}-${day}T23:59:59`);
    ...
    const finalAction = (isExpired || !item.isActive) ? "Hết hạn" : "Dùng ngay";
